...
func main() {
	...
	for gameOver != true {
		...
		player := 0
		if turnNumber%2 == 1 {
			fmt.Println("Spiller 1 sin tur")
			player = 1
		} else {
			fmt.Println("Spiller 2 sin tur")
			player = 2
		}
...
func executePlayerMove(moveInt int, player int, b [9]int) [9]int {
	...
	if b[moveInt] != 0 {
		fmt.Println("Velg et ledig sted.")
		moveInt = askForPlay()
	...
	for moveInt > 9 {
		fmt.Println("Vennligst tast inn et nummer under.")
		moveInt = askForPlay()
	}
